...
public class Branch {
	...
	public void grow(){
		...
		if(Math.random() <= 0.5){
			...
			changeYPoint((Math.sin(direction)));
		}
	...
	public Point randomPoint(int start) {
		...
		if(start < xPoints.size()) {
			...
			int rand = start + (int) (Math.random() * (double) (xPoints.size() - start - 1));
			
			return new Point(xPoints.get(rand).intValue(), yPoints.get(rand).intValue());
		}
